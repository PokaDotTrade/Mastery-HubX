...
const Layout: React.FC<LayoutProps> = ({ 
  children, activeTab, setActiveTab, title, description, focusLevel, focusXP, xpToNext, onLogout, isSidebarCollapsed, toggleSidebar
}) => {
    ...
    <div className="min-h-screen bg-[#080d0a] text-[#f8faf9] flex justify-center">
      <div className="w-full max-w-[1800px] flex flex-col md:flex-row relative">
        ...
        <aside className={`hidden md:flex flex-col h-screen sticky top-0 border-r border-white/5 p-6 space-y-6 bg-[#080d0a] shrink-0 transition-all duration-300 ease-in-out relative ${isSidebarCollapsed ? 'w-28' : 'w-80'}`}>
          ...
          <div className={`flex items-center gap-4 mb-4 transition-all duration-300 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
          ...
          <nav className="flex-1 flex flex-col gap-1 no-scrollbar">
            ...
            <DesktopNavItem icon="checklist" label="Habit Engine" active={activeTab === 'habits'} onClick={() => setActiveTab('habits')} isCollapsed={isSidebarCollapsed} />
            <DesktopNavItem icon="layers" label="Skill Stack" active={activeTab === 'mastery'} onClick={() => setActiveTab('mastery')} isCollapsed={isSidebarCollapsed} />
          ...
          <div className="mt-auto space-y-4">
             <div className={`glass rounded-[32px] border-emerald-400/10 transition-all duration-300 ${isSidebarCollapsed ? 'p-3' : 'p-6'}`}>
                {!isSidebarCollapsed && (
                  <div className="flex justify-between items-center mb-4 animate-in fade-in duration-300">
                ...
                {!isSidebarCollapsed && (
                  <div className="flex justify-between items-center mt-4 animate-in fade-in duration-300">
              ...
              <button 
                onClick={onLogout}
                className={`w-full flex items-center gap-4 p-4 rounded-[20px] transition-all hover:bg-rose-500/5 group text-slate-500 hover:text-rose-400 ${isSidebarCollapsed ? 'justify-center' : ''}`}
              ...
              >
                ...
                {!isSidebarCollapsed && <span className="text-[11px] font-black uppercase tracking-widest whitespace-nowrap">Terminate Link</span>}
              </button>
          </div>
        </aside>
...
const DesktopNavItem = ({ icon, label, active, onClick, isCollapsed }: { icon: string, label: string, active: boolean, onClick: () => void, isCollapsed: boolean }) => (
  ...
  >
    ...
    {!isCollapsed && <span className="text-[13px] font-bold font-body uppercase tracking-[0.15em] whitespace-nowrap">{label}</span>}
  </button>
);
